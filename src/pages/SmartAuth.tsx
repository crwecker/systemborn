...
import { checkUserExists, requestMagicLink, requestSignInLink } from '../services/auth';
...
export function SmartAuth() {
        ...
        if (result.exists) {
          ...
          if (response.success) {
            ...
            if (response.verifyUrl) {
              setSuccessMessage(`Magic link created! Click here to verify: ${response.verifyUrl}`);
            } else {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.verifyUrl) {
          ...
          setSuccessMessage('Welcome! Check your email for the magic link to complete your registration.');
        }
        setMode('success');
      } else {
        setMessage({
  ...
  const getTitle = () => {
    switch (mode) {
      ...
      default: return 'Join LitRPG Academy';
    }
  };

  const getDescription = () => {
    switch (mode) {
      case 'signup': return 'Complete your sign up to discover your next great read!';
      case 'success': return 'We\'ve sent you a magic link to sign in.';
      default: return 'Enter your email to sign in or to create an account.';
    }
  };
