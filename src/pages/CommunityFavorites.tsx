                  ...
                  {tierGroups.SSS.length > 0 && (
                    <div>
                      <div className="flex items-center mb-3">
                        <div className={`px-3 py-1 rounded text-white font-bold text-sm ${TIER_CONFIG.SSS.color}`}>
                  ...
                  {(tierGroups.SS.length > 0 || tierGroups.S.length > 0) && (
                    <div className="space-y-3">
                      {(['SS', 'S'] as const).map(tier => {
                        ...
                        return (
                          <div key={tier} className="relative p-3 rounded-lg border-2 border-gray-200 bg-white">
                            ...
                            <div className="flex gap-2 justify-center h-24">
                              {slots.map((bookTier, index) => (
                                <div key={bookTier?.id || `empty-${tier}-${index}`} className="w-16 h-full">
                                  {bookTier ? (
                                    <div className="relative w-full h-full">
                                      {bookTier.book && (
                                        <SimplifiedBookCard book={bookTier.book} tier={bookTier.tier} compact={true} />
                                      )}
                                    </div>
                                  ) : (
                        ...
                        {(['SS', 'S'] as const).map(tier => {
                          ...
                          return (
                            <div key={tier} className="relative p-3 rounded-lg border-2 border-gray-200 bg-white">
                              ...
                              <div className={`absolute top-2 left-2 px-2 py-1 rounded text-white font-bold text-xs ${config.color} z-10`}>
                                {config.name}
                              </div>
                              ...
                              <div className="flex gap-2 justify-center h-32">
                                {slots.map((bookTier, index) => (
                                  <div key={bookTier?.id || `empty-${tier}-${index}`} className="w-20 h-full">
                                    {bookTier ? (
                                      <div className="relative w-full h-full">
                                        {bookTier.book && (
                                          <SimplifiedBookCard book={bookTier.book} tier={bookTier.tier} compact={true} />
