...
export const BookSearchModal: React.FC<BookSearchModalProps> = ({
  ...
  const handleAddBook = (book: Book) => {
    ...
    if (existingTier) {
      alert(
        `This book is already in your ${TIER_CONFIG[existingTier.tier].name}!`
      )
      return
    }
    ...
    const currentBooksInTier = userTiers.filter(
      tier => tier.tier === targetTier
    ).length

    if (tierConfig.maxBooks && currentBooksInTier >= tierConfig.maxBooks) {
      alert(
        `${tierConfig.name} is full! Maximum ${tierConfig.maxBooks} books allowed.`
      )
      return
    }
  ...
  const filteredResults = searchResults.filter(
    book => !userTiers.some(tier => tier.bookId === book.id)
  )
      ...
      <div className='bg-white rounded-lg max-w-2xl w-full max-h-[80vh] overflow-hidden'>
        ...
        <div className='flex items-center justify-between p-4 border-b'>
          <div>
            ...
            <div className='flex items-center gap-2 mt-1'>
              ...
              <div
                className={`px-2 py-1 rounded text-white text-xs font-bold ${TIER_CONFIG[targetTier].color}`}>
        ...
        <div className='p-4 border-b'>
          ...
          <p className='text-sm text-gray-500 mt-2'>
            Type at least 3 characters to search. Books already in your tiers
            are filtered out.
          </p>
        </div>
          ...
          {debouncedQuery.length < 3 ? (
            ...
            <div className='text-center py-8 text-gray-500'>
              ...
              <p className='text-sm mt-1'>
                Try different search terms or check if the books are already in
                your tiers
              </p>
            </div>
          ) : (
                ...
                <div
                  ...
                  <div className='flex-1 min-w-0'>
                    ...
                    <div className='flex items-center gap-3 text-xs text-gray-500 mb-2'>
                      ...
                      <span>
                        {(book.stats?.followers || 0).toLocaleString()}{' '}
                        followers
                      </span>
                    </div>
